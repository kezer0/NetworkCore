...
public final class PlayerDataManager {
    ...
    public static void handleJoin(final Player player) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(NetworkCore.getInstance(), () -> {
            try {
                ...
                NetworkCore.getInstance().getLogger().log(Level.SEVERE, "Failed to load network player data for " + name, ex);
            }
        });
    }
    ...
    public static void saveAsync(PlayerData data) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(NetworkCore.getInstance(), () -> {
            ...
            catch (Exception ex) { NetworkCore.getInstance().getLogger().log(Level.WARNING, "Failed to save network player data for " + data.getUuid(), ex); }
        });
    }
    ...
    public static void saveAll() {
        ...
        for (Map.Entry<UUID, PlayerData> entry : CACHE.entrySet()) {
            ...
            catch (Exception ex) { NetworkCore.getInstance().getLogger().log(Level.WARNING, "Failed to save network player data for " + entry.getKey(), ex); }
        }
    }
